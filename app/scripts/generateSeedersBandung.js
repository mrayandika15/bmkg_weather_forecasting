...
// New: Generate seeders for Bandung + non-Bandung to reach 1k
function generateSeedersBandung() {
  const bandungNames = ["Bandung", "Kota Bandung", "Bandung Barat"];
  // Filter Bandung rows
  const bandungRows = processedLocations.filter((loc) =>
    bandungNames.includes(loc.kotkab)
  );
  // Filter non-Bandung rows
  const nonBandungRows = processedLocations.filter(
    (loc) => !bandungNames.includes(loc.kotkab)
  );
  // How many more needed to reach 1k?
  const needed = Math.max(0, 1000 - bandungRows.length);
  const selectedNonBandung = nonBandungRows.slice(0, needed);
  const finalRows = [...bandungRows, ...selectedNonBandung];

  // Write to bandung_seeders directory
  const bandungSeedersDir = path.join(
    __dirname,
    "../../db/seeders/bandung_seeders"
  );
  fs.mkdirSync(bandungSeedersDir, { recursive: true });

  // Province, city, district, subdistrict extraction (same as snowflake logic)
  const provinces = new Map();
  const cities = new Map();
  const districts = new Map();
  finalRows.forEach((location) => {
    const province_code = location.adm1;
    if (!provinces.has(province_code)) {
      provinces.set(province_code, {
        province_code: province_code,
  ...
  const cityValues = Array.from(cities.values());
  const districtValues = Array.from(districts.values());
  const subdistrictValues = finalRows.map((location) => ({
    subdistrict_code: location.adm4.split(".").pop(),
    subdistrict_name: location.desa,
    district_code: location.adm3.split(".").pop(),
    latitude: location.lat,
  ...
  // Seeder file contents (same as snowflake, but using finalRows)
  const provinceSeederContent = `const db = require('${dbConfigPath}');
const provinces = ${JSON.stringify(provinceValues, null, 2)};
async function seedProvinces() {
  try {
  ...
  try {
    ...
    for (const city of cities) {
      ...
      if (provinceRes.rows.length > 0) {
        ...
        console.error(\`Province with code \\${
          city.province_code
        } not found for city \\${city.city_name}\`);
      }
    }
    console.log('Cities seeded successfully!');
  } catch (error) {
  ...
  try {
    ...
    for (const district of districts) {
        ...
        if (cityRes.rows.length > 0) {
            ...
            console.error(\`City with code \\${
              district.city_code
            } not found for district \\${district.district_name}\`);
        }
    }
    console.log('Districts seeded successfully!');
  } catch (error) {
...
async function seedSubdistricts() {
  try {
    console.log(\`Starting to seed \\${
      subdistricts.length
    } subdistricts for bandung_seeders...\`);
    for (const subdistrict of subdistricts) {
      const districtRes = await db.query('SELECT district_id FROM forecasting.dim_district WHERE district_code = $1', [subdistrict.district_code]);
      if (districtRes.rows.length > 0) {
        const district_id = districtRes.rows[0].district_id;
        ...
        console.error(\`District with code \\${
          subdistrict.district_code
        } not found for subdistrict \\${subdistrict.subdistrict_name}\`);
      }
    }
    console.log('Subdistricts seeded successfully!');
  } catch (error) {
...
module.exports = seedSubdistricts;`;
  ...
  // Write files
  fs.writeFileSync(
    path.join(bandungSeedersDir, "provinceSeeder.js"),
    provinceSeederContent
  );
  fs.writeFileSync(
    path.join(bandungSeedersDir, "citySeeder.js"),
    citySeederContent
  );
  fs.writeFileSync(
    path.join(bandungSeedersDir, "districtSeeder.js"),
    districtSeederContent
  );
  fs.writeFileSync(
    path.join(bandungSeedersDir, "subdistrictSeeder.js"),
    subdistrictSeederContent
  );
...
const seedDistricts = require('./districtSeeder');
const seedSubdistricts = require('./subdistrictSeeder');
module.exports = [seedProvinces, seedCities, seedDistricts, seedSubdistricts];`;
  fs.writeFileSync(path.join(bandungSeedersDir, "index.js"), indexContent);

  console.log(`Bandung seeders generated successfully in ${bandungSeedersDir}`);
}

// Export for CLI usage
if (require.main === module && process.env.BANDUNG_SEEDERS) {
  generateSeedersBandung();
}
